...
Our Coq library is parametric in an underlying type of sets, and hence we need an interface to interact with various set implementations.  Our interface is based monads~\cite{monad}, which are a useful interface for many kinds of collections, including sets~\cite{monad}.  The operations, but not the required axioms, of our monadic interface are shown in Figure~\ref{fig:chaseable-functor}.  Intuitively, in a set monad these operations are:
\begin{itemize}
...
\item \coqe{Mstr} is the cartesian product of two sets.  (In an arbitrary monad we require a {\it strength}~\cite{BW}.)
...
Since our queries are simply Coq terms, explicitly referencing binders by name can be quite difficult, and is not very extensible (i.e., not invariant under $\alpha$-equivalence).
Instead, we will encode our mapping of binders as a function between the types being bound.
...
Mimpl (Mmap ?h P) F
\end{coq}
In the above \coqe{Mimpl}, is defined as subset containment (or rather, the analog of subset containment for arbitrary monads), and \coqe{Mmap} expresses the application of a substitution.  The first task is to break \coqe{F} down into atomic units which correspond to the binders.
...
Once we get to this step, \coqe{h} is exactly one of the morphisms constructed by the previous step.
...
forall p, p.age > 16 && p.age < 18 = true ->
          p.age < 21 = true
\end{coq}
Coq's \coqe{omega} tactic performs such arithmetic reasoning.
...
When we fail to chase with a particular ED, it is no longer necessary to backtrack into the application of that ED until we have chased with another ED.
...
To solve it, we ``back chase'' the left-hand side of the equivalence and try to determine if it is homomorphically equivalent to the right-hand side.  \coqe{minimize_keep} is the fallback case.
\begin{coq}
Lemma minimize_keep
...
Proof. ... Qed.
\end{coq}
If we can not find a way to re-construct the removed binder from the other binders of the query, then we cannot remove that binder, but we can still optimize the rest of the query (represented by quantifying over any value from the relation and ensuring that the rest of the query is still valid).
...
In this paper we have implemented a verifying query optimizer as a Coq tactic.  Doing so has several trade-offs compared to a more traditional approach that would implement a query optimizer as a Coq program (e.g.,~\cite{coqdb}).
...
Much of this benefit is due to the flexibility of working indirectly on Coq's underlying terms.
...
A non-tactic implementation would have to adjust its concrete term representations to support the more sophisticated structure of nested queries and then update all of its algorithms to work on the new nested representation.
Similarly, we explicitly support arbitrary Coq computation within where clauses.
While the examples in this paper use only equality ($=$) and less-than ($<$) in where clauses, there is nothing preventing us from reasoning about more complex operations (e.g., computing the length of strings).

Another benefit of the tactic-based approach is that we are able to re-use much of Coq's underlying  infrastructure.
...
We have found, in particular, that the new backtracking proof search facilities, namely \coqe{+} and dependent goals, introduced in Coq 8.5, were extremely useful in building our optimizer.
Without this backtracking feature, tactics can not backtrack between different goals which initially forced us to more meticulously maintain goals in addition to forcing us to code in continuation passing style, rather than a more direct style.
...
The {\tt normalize} task (not discussed) is the time it takes to normalize Coq terms, using the monad laws, into the flat form \coqe{query P C R}.
The {\tt chase} and {\tt minimize} phase are the phases described in Sections~\ref{sec:chase-step} and~\ref{sec:minimization}.
...
For example, queries need to be manipulated indirectly through applying and rewriting lemmas; backtracking search is necessary to explore multiple potential optimization paths; and traditional first-order phrasings of syntax in terms of binders with concrete variable names need to be translated to manipulations of environments.  We hope that our solutions to these issues in this paper will allow others to create their own query (and program) optimizers as Coq tactics.  

While not currently competitive with more traditional programming languages in terms of speed, \ltac{} is continually improving and features prominently in the development of large-scale verified software systems such as Idris~\cite{brady2013idris}.  Moreover, recent work~\cite{malecha2015thesis,devriese2013tsmp,vanderwalt2013engineering-reflection-agda} has shown how tactic-based programming can be made first-class (i.e., reified into Coq proper) by exploiting dependent types.
...
This next wave of tactic-based languages and libraries are already demonstrating substantial performance improvements and it is likely that it won't be too long before tactic-based optimization is a viable design pattern for language-integrated query systems.
