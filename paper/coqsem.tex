...
In addition to these operators, \coqe{DataModel} also provides a relation \coqe{Mimpl m$_1$ m$_2$}, which states that \coqe{m$_1$} is a subset of \coqe{m$_2$}.
We extend subset to equivalence (written \coqe{Meq m$_1$ m$_2$}) using mutual containment.
...
Since no more optimization is possible, we solve the goal by appealing to the reflexivity of \coqe{Meq} which implicitly instantiates \coqe{?n} with `\coqe{x <- Movie ; Mret x}'.
...
However, since the above reasoning is occurring at the top level of the goal, we could also use \ltac's more primitive \coqe{eapply} tactic with \coqe{Mbind_dedup} to yield a proof in a single step.
...
Proof. tauto. Qed.
\end{coq}
Both of these lemmas are trivial tautologies that can be proven by the \coqe{tauto} tactic; however, we have found that separating the task of crafting the lemma from its proof dramatically improves our ability to evolve our tactics.
...
Ltac chase solver :=
  repeat first
    ...
    | reflexivity ].
\end{coq}
where \coqe{transitive_refine_conditional} expresses transitivity under the implication:
\begin{coq}
...
Proof. ... Qed.
\end{coq}
Here, \coqe{repeat} computes the transitive closure of the chase step by running the rest of the tactic until the goal is solved or the tactic fails.
\coqe{first [ a | b ]} runs \coqe{a} and, if \coqe{a} fails, runs \coqe{b}.
The first branch uses a transitivity lemma (\coqe{transitive_refine_conditional}) to break the goal into two sub-goals.
