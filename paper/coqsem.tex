...
Several authors have proposed more richly typed tactic-based programming languages, noteably Mtac~\cite{} and VeriML~\cite{}.
...
For example, rather than writing tactics to compute the chase and then use Coq's logic to prove the procedure sound similar to the work of ???~\cite{}.
This can yield a substantial improvement in performance.
For example, optimizing the Movie query from beginning to end is practically instantaneous using Coq's \coqe{vm_compute} reduction mechanism.
