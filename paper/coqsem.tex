...
\caption{Abstractly representing the functors \coqe{M} that correspond to ``chaseable'' queries.}
\label{fig:chaseable-functor}
\end{figure}


The interface is based on Haskell's interface for monads and functors.
\coqe{Mret v} is the function that corresponds to the singleton relation containing only the value \coqe{v}.
\coqe{Mbind m k} is the function that corresponds to the union of all relations \coqe{k x} for any \coqe{x} in the \coqe{m} relation.
Following Haskell convention, we will write \coqe{x <- m ; k} for \coqe{Mbind x (fun x => k)} where \coqe{x} occurs free in \coqe{k}.
Next is \coqe{Mzero} which corresponds to the empty relation.
\coqe{Mplus} is the cross-product of the two instances.
\coqe{Mguard P m} is the same as \coqe{Mzero} if \coqe{P} is false and otherwise returns \coqe{m}.
...
This work flow is built into the semantic of \ltac{} because \ltac{} tactics run on particular proof obligations or goals (e.g. \coqe{x + y = y + x}).
...
We could simply pick \coqe{?n} to be the full query \coqe{x <- Movie ; y <- Movie ; ret x}, but that would require us to do an unnecessary join.
...
Proof. ... Qed.
\end{coq}
First, because \coqe{Meq} is an equivalence relation, we can use \ltac's \coqe{setoid_rewrite} to rewrite using the relation.
Running \coqe{setoid_rewrite <- Mbind_dedup} on the goal above results in the following:
\begin{coq}
Meq ?n (x <- Movie ; ret x)
\end{coq}
...
When building optimization procedures in \ltac{}, \coqe{apply} often ammounts to the core reasoning tool.
...
Meq ?n (Mplus a b)
\end{coq}
If we wish to optimize \coqe{a} and \coqe{b} independently  then we can apply the following lemma.
\begin{coq}
...
\end{coq}
When we apply this lemma to the above goal, we expect \coqe{m1} and \coqe{m2} to be completely determined but we expect \coqe{m1'} and \coqe{m2'} to be new unification variables.
Because the tactic will introduce new unification variables, we use the \coqe{eapply} tactic.
Running \coqe{eapply opt_plus} on the above goal yields the following sub-goals
\begin{center}
\begin{tabular}{ccc}
...
As our tactics continue to fill in \coqe{?m1'} and \coqe{?m2'}, \coqe{?n} will be automatically updated to reflect these changes.
...
For example, we can optimize with the \coqe{title_director} embedded dependency using the following goal structure.
\begin{coq}
title_director ->
...
For example, if we EDs \coqe{A}, \coqe{B}, and \coqe{C} are all true, and we wish to optimize the query \coqe{q}, the goal posed to the tactic is the following:
\begin{coq}
A /\ B /\ C -> Meq ?q q
\end{coq}
...
The second is that in doing so, we must construct the morphism \coqe{h} so that we can use it when we check the side-condition.
...
The first task is to break \coqe{F} down into atomic units which, essentially, correspond to the binders.
...
This matching is essentially the same as the ED search procedure except that, as above, we must record the way to re-constuct the \coqe{h} function.
\begin{coq}
