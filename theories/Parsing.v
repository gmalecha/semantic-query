...
Section syntax.
  ...
  Fixpoint find (s : string) (ls : list (string * row_type)) : option row_type :=
    match ls with
    | nil => None
  ...
  Fixpoint find_to_member (ls : list (string * row_type)) (s : string) val {struct ls}
    ...
    match ls as ls return find s ls = Some val -> Member.member val (List.map snd ls) with
  ...
  Fixpoint compile_e (vars : list (string * row_type)) T (q : expr_ast)
    ...
    | WT_Proj v f T pf =>
      ...
      | Some X => fun pf_f pf_v => @Proj _ T _ (find_to_member _ _ pf_v) (to_member _ _ pf_f)
      end pf eq_refl
    end.

  Fixpoint compile_q' (vars : list (string * row_type)) (q : query_ast)
           (bs : binds_type tbls (List.map snd vars)) (fs : filter_type (List.map snd vars))
           (wt : WT_query vars q)  : tableaux tbls :=
    match wt with
    | WT_Ret => {| types := List.map snd vars ; binds := bs ; filter := fs |}
    | @WT_Bind x ti t_t q' pf_eq pf_wt =>
      @compile_q' ((x,t_t) :: vars) q' (Hcons (to_member _ _ pf_eq) bs) (@weaken_filter _ _ fs) pf_wt
    | @WT_Guard l r q' T wt_l wt_r wt_q =>
      @compile_q' vars q' bs (@existT2 _ _ _ T (compile_e wt_l) (compile_e wt_r) :: fs) wt_q
    end.
