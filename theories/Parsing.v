...
Section syntax.
  ...
  Fixpoint compile_e (vars : list row_type) T (q : expr_ast)
           (wt : WT_expr vars q T) {struct wt} : expr vars T :=
    match wt with
    | WT_Proj v f T pf =>
  ...
  Fixpoint compile_q' (vars : list row_type) (q : query_ast)
    ...
    | @WT_Bind x ti t_t q' pf_eq pf_wt =>
      @compile_q' (t_t :: vars) q' (Hcons (to_member _ _ pf_eq) bs) (@weaken_filter vars _ fs) pf_wt
